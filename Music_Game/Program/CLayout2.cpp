...
void CLayout2::LoadData(string entry_name)
{
	...
	for (auto itr = list_str.begin(); itr != list_str.end(); itr++)
	{
		...
		if (itr->find("TextureName:") != -1)
		{
			str_split = itr->substr(12);
			m_str_tex = str_split;
		}
		...
		else if (itr->find("ReferenceY:") != -1)
		{
			...
			{
				lyt.reference_y = ref_Top;
			}
			else if (str_split.find("Bottom") != -1)
			{
				lyt.reference_y = ref_Bottom;
			}
			else if (str_split.find("Center") != -1)
			{
				lyt.reference_y = ref_Center;
			}
		}
		else if (itr->find("SrcX:") != -1)
		{
			str_split = itr->substr(5);
			lyt.rectangle.m_x = stoi(str_split);
		}
		else if (itr->find("SrcY:") != -1)
		{
			str_split = itr->substr(5);
			lyt.rectangle.m_y = stoi(str_split);
		}
		else if (itr->find("SrcW:") != -1)
		{
			str_split = itr->substr(5);
			lyt.rectangle.m_width = stoi(str_split);
		}
		else if (itr->find("SrcH:") != -1)
		{
			str_split = itr->substr(5);
			lyt.rectangle.m_height = stoi(str_split);
		}
		else if (itr->find("LoopX:") != -1)
		{
			str_split = itr->substr(6);
			lyt.loop_x = stob(str_split);
		}
		//アニメ情報をエントリに追加する
		else if (itr->find("LoopY:") != -1)
		{
			str_split = itr->substr(6);
			lyt.loop_y = stob(str_split);
		}
		else if (itr->find("FlipX:") != -1)
		{
			str_split = itr->substr(6);
			lyt.flip_x = stob(str_split);
		}
		//アニメ情報をエントリに追加する
		else if (itr->find("FlipY:") != -1)
		{
			str_split = itr->substr(6);
			lyt.flip_y = stob(str_split);
		}
	
		else if (itr->find("ScaleX:") != -1)
		{
			str_split = itr->substr(7);
			lyt.scale_x = stof(str_split);
		}
		//アニメ情報をエントリに追加する
		else if (itr->find("ScaleY:") != -1)
		{
			str_split = itr->substr(7);
			lyt.scale_y = stof(str_split);
		}
		else if (itr->find("RotateZ:") != -1)
		{
			str_split = itr->substr(8);
			lyt.rotate_z = stof(str_split);
		}
		else if (itr->find("X:") != -1)
		{
			str_split = itr->substr(2);
			lyt.pos.x = stoi(str_split);
		}
		else if (itr->find("Y:") != -1)
		{
			str_split = itr->substr(2);
			lyt.pos.y = stoi(str_split);
		}
		else if (itr->find("Blend:") != -1)
		{
			...
			if (str_split.find("Normal") != -1)
				lyt.blend_mode = E_BLEND_MODE::NORMAL;
			else if (str_split.find("AdditionAlpha") != -1)
				lyt.blend_mode = E_BLEND_MODE::ADDITIONALPHA;
			else if (str_split.find("Addition") != -1)
				lyt.blend_mode = E_BLEND_MODE::ADDITION;
		}
	}
}
